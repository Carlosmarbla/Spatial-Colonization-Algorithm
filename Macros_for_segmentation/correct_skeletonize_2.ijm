...
//FUNCTIONS 
// This function opens the classified image, opens the cropped and corrected image together to continue the manual correction of the tracheal tree
function Black_FLAG2(input, input2, output, output2, filename) {
...
//	open(input + filename); // open cropped image to compare and then close it
	...
	setTool( "Pencil Tool" ); // change this to pencil an set black or white colour before
	 // open cropped image to compare and overlay with some transparency, and then close it once manual curation is finish before saving the corrected image
...
ARRAY = getFileList(OUT_BnW_corrected);
for (i = 0; i < ARRAY.length; i++)
	Black_FLAG2(OUT_BnW_corrected, OUT_cropped_image, OUT_BnW_class, OUT_BnW_corrected, ARRAY[i]);
